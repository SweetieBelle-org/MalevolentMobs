...
public class MMobListener implements Listener {
    ...
    public void onMobSpawn(final CreatureSpawnEvent event) {
        ...
        final String type = Common.getEntityType(event.getEntity());
        ...
        if (commandSpawn || random.nextFloat() < chance) {
            ...
            final List<String> blockedWorlds = MMobPlugin.getSettings().getStringList("Spawns.blockedWorlds");
            final String currentWorld = event.getEntity().getWorld().getName();
            ...
            if (!commandSpawn && !MMobSettings.isDebugmode) {
                final List<Player> players = Common.getPlayersInRange(event.getLocation(), MMobPlugin.getSettings().getFloat("General.Mobs.minSpawnDistance"));
                if (players.size() > 0)
                    return;
            }
    ...
    public void onMobDie(final EntityDeathEvent event) {
        final LivingEntity entity = event.getEntity();
        if (MobHandler.isMobMalevolent(entity))
            MobHandler.getMalevolentMob(entity).onDie(event);
    }

    /** Handle the events where the malevolent mobs are damaged by a skill */
    /* @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true) public void onMobDamageTaken(SkillDamageEvent event) {} */
    ...
    public void onMobDamageGiven(final EntityDamageByEntityEvent event) {
        final Entity attacker = Common.getAttacker(event);
    ...
    public void onMobDamageTaken(final EntityDamageEvent event) {
    ...
    @EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
    public void onMobHealed(final EntityRegainHealthEvent event) {
        // Ignore mobs that definitely aren't malevolent for the rest
        if (!(event.getEntity() instanceof LivingEntity))
            return;
        final LivingEntity entity = (LivingEntity) event.getEntity();
    ...
    public void onPlayerMoveEvent(final PlayerMoveEvent event) {
        ...
        final Vector deltaPosition = event.getTo().clone().subtract(event.getFrom()).toVector();
        playerVelocities.put(player.getName(), deltaPosition);
    }

    /** Used to track the bow itemstack that was used to fire an arrow */
    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onPlayerFireArrow(final EntityShootBowEvent event) {
        if (event.getEntity() instanceof Player && event.getProjectile() instanceof Arrow) {
            ...
            final MetadataValue meta = new FixedMetadataValue(MMobPlugin.getInstance(), event.getBow());
    ...
    public static void setSpawnDenyFlag(final boolean denySpawns) {
        MMobListener.denySpawns = denySpawns;
    }

    /** Sets if the current mob spawn is due to a command or not */
    public static void setSpawnCommandFlag(final boolean commandSpawn) {
        MMobListener.commandSpawn = commandSpawn;
    }

    /** Returns the velocity of a player */
    public static Vector getPlayerVelocity(final Player player) {
        // Need a valid player
        if (player == null)
