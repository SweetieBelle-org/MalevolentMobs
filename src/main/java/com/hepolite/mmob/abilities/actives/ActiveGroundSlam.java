...
public class ActiveGroundSlam extends Active {
    ...
    public ActiveGroundSlam(final MalevolentMob mob, final float scale) {
    ...
    public void cast(final LivingEntity target) {
        final LivingEntity self = mob.getEntity();
        inAirTimer = 0;

        // The attacker will leap towards the target, if it has one
        if (target != null) {
            final Location difference = target.getLocation().subtract(self.getLocation()).multiply(0.175);
    ...
    public void onTick() {
        ...
        if (!onGround && mob.getEntity().isOnGround()) {
            if (inAirTimer >= 10) {
                ...
                final List<LivingEntity> entities = Common.getEntitiesInRange(mob.getEntity().getLocation(), maxRange);
                for (final LivingEntity entity : entities) {
                    if (!entity.isOnGround())
                        continue;

                    float multiplier = 1.0f;
                    final float distance = (float) mob.getEntity().getLocation().distance(entity.getLocation());
