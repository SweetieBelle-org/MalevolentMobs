...
public class ActiveKidnap extends Active {
    ...
    public ActiveKidnap(final MalevolentMob mob, final float scale) {
    ...
    public boolean canCast(final float healthFactor, final float distanceToTarget, final LivingEntity target) {
        ...
        final List<Player> playersNearby = Common.getPlayersInRange(mob.getEntity().getLocation(), 25.0f);
        final List<Monster> entitiesNearby = Common.getMonstersInRange(mob.getEntity().getLocation(), 25.0f);
