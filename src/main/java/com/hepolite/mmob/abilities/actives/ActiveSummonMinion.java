...
public class ActiveSummonMinion extends ActiveTick {
    ...
    public ActiveSummonMinion(final MalevolentMob mob, final float scale) {
    ...
    protected void applyTickEffect() {
        ...
        /*
         * EntityType type = Common.getEntityType(entityTypes.get(random.nextInt(entityTypes.size()))); EntityType type = null; try { String name = entityTypes.get(random.nextInt(entityTypes.size())).toUpperCase(); type = EntityType.valueOf(name); } catch (Exception exception) { Log.log("Failed to obtain the entity type when summoning a minion!", Level.WARNING); Log.log(exception.getLocalizedMessage()); } if (type == null) return;
         */

        // Get the type to spawn
        final String type = entityTypes.get(random.nextInt(entityTypes.size()));
