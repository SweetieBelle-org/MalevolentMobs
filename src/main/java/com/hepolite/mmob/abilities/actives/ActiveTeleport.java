...
public class ActiveTeleport extends Active {
    ...
    public ActiveTeleport(final MalevolentMob mob, final float scale) {
    ...
    public void cast(final LivingEntity target) {
        ...
        final Location mobLocation = mob.getEntity().getLocation();
        final Location entityLocation = target.getLocation();
        ...
        if (requestSafeTeleport && timeWithoutRequestingSafeTeleport < 50) {
            ...
            final double angle = 2.0 * Math.PI * random.nextDouble();
            final double distance = minDistance + (maxDistance - minDistance) * random.nextDouble();
            targetLocation = mobLocation.add(distance * Math.cos(angle), 0.0, distance * Math.sin(angle));
        } else {
            final List<Player> nearbyPlayers = Common.getPlayersInRange(mobLocation, maxSearchDistance);
            ...
            if (nearbyPlayers.size() / (1.0f + healthFactor) <= 1.75f) {
                final double angle = 2.0 * Math.PI * random.nextDouble();
                targetLocation = entityLocation.add(minDistance * Math.cos(angle), 0.0, minDistance * Math.sin(angle));
            } else {
                final double angle = 2.0 * Math.PI * random.nextDouble(); // TODO: Make this the angle that points away from the target, plus some random factor
                final double distance = minDistance + (maxDistance - minDistance) * random.nextDouble();
                targetLocation = mobLocation.add(distance * Math.cos(angle), 0.0, distance * Math.sin(angle));
            }
        }
