...
public class ActiveWebbing extends ActiveArea {
    ...
    public ActiveWebbing(final MalevolentMob mob, final float scale) {
    ...
    @Override
    public void applyEffect(final Location areaLocation) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            final Location location = areaLocation.clone();
                ...
                for (int z = -radius; z <= radius; z++)
                    if (x * x + z * z + random.nextInt(3) < radius * radius) {
                        ...
                        for (int j = 0; i >= -radius; j--) {
                            final Block block = location.getWorld().getBlockAt(location.getBlockX() + x, y + j, location.getBlockZ() + z);
