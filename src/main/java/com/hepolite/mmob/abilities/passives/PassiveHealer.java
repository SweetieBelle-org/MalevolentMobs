...
public class PassiveHealer extends Passive {
    ...
    public PassiveHealer(final MalevolentMob mob, final float scale) {
    ...
    public void onHealed(final EntityRegainHealthEvent event) {
        ...
        if (healAmount > 0.0) {
            final List<Monster> monsters = Common.getMonstersInRange(mob.getEntity().getLocation(), range);
            for (final Monster monster : monsters) {
                ...
                if (monster.getHealth() < monster.getMaxHealth()) {
                    final double neededHealth = monster.getMaxHealth() - monster.getHealth();
