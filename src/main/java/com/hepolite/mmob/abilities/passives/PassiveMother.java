...
public class PassiveMother extends PassiveTick {
    ...
    public PassiveMother(final MalevolentMob mob, final float scale) {
    ...
    protected void applyTickEffect() {
        ...
        final List<Player> nearbyPlayers = Common.getPlayersInRange(mob.getEntity().getLocation(), maxSearchDistance);
        if (nearbyPlayers.size() == 0)
            return;
        final List<Monster> nearbyMonsters = Common.getMonstersInRange(mob.getEntity().getLocation(), maxSearchDistance);
