...
public class ItemEffectHandler {
    ...
    private static void registerItemEffect(final ItemEffect effect) {
    ...
    public static ItemEffect getItemEffect(final String name) {
    ...
    public static void loadItemEffectsFromConfig() {
        for (final ItemEffect effect : effectMap.values())
    ...
    public static void onPlayerTick(final Player player) {
        ...
        for (final ItemStack item : items)
            if (item != null)
                for (final ItemEffect effect : getItemEffects(item))
    ...
    public static void onPlayerLeftClick(final PlayerInteractEvent event, final Player player) {
    ...
    public static void onPlayerRightClick(final PlayerInteractEvent event, final Player player) {
        final ItemStack item = event.getItem();
        for (final ItemEffect effect : getItemEffects(item))
    ...
    public static void onPlayerTakeDamage(final EntityDamageEvent event, final Player player) {
        final EntityEquipment equipment = player.getEquipment();
        final ItemStack items[] = { equipment.getItemInMainHand(), equipment.getItemInOffHand(), equipment.getHelmet(), equipment.getChestplate(), equipment.getLeggings(), equipment.getBoots() };
    ...
    public static void onPlayerDealDamage(final EntityDamageByEntityEvent event, final Player player) {
        // Get the itemstack that corresponded to the attack
        ItemStack item = null;
        if (event.getDamager() instanceof Arrow) {
            final List<MetadataValue> metas = event.getDamager().getMetadata("[MMob]originBow");
            if (metas.size() != 0)
    ...
    public static void onPlayerFireArrow(final EntityShootBowEvent event, final Player player) {
    ...
    public static void onBlockBreak(final BlockBreakEvent event, final Player player) {
    ...
    public static boolean addItemEffect(final ItemStack itemStack, final ItemEffect itemEffect) {
        // Effect must exist and must be enabled
        final Settings settings = SettingsItemEffects.getConfig(itemEffect.getName());
        if (settings == null || !settings.getBoolean("enable"))
            return false;

        final NBTTag tag = NBTAPI.hasTag(itemStack) ? NBTAPI.getTag(itemStack) : new NBTTag();
        final NBTList mmob = tag.hasKey("mmob") ? tag.getList("mmob") : new NBTList();
        final NBTTag display = tag.hasKey("display") ? tag.getTag("display") : new NBTTag();
        final NBTList lore = display.hasKey("Lore") ? display.getList("Lore") : new NBTList();

        // Check if the effect to add is incompatible with something else on the item. If it is, don't add it
        for (final String effect : itemEffect.getIncompatibleEffects())
    ...
    public static void removeItemEffects(final ItemStack itemStack) {
        final NBTTag tag = NBTAPI.hasTag(itemStack) ? NBTAPI.getTag(itemStack) : null;
    ...
    public static List<ItemEffect> getItemEffects(final ItemStack itemStack) {
        final List<ItemEffect> list = new LinkedList<ItemEffect>();

        final NBTTag tag = NBTAPI.getTag(itemStack);
        if (tag == null || !tag.hasKey("mmob"))
            return list;
        final NBTList mmob = tag.getList("mmob");

        for (int i = 0; i < mmob.size(); i++) {
            final ItemEffect itemEffect = readItemEffect(mmob.getString(i));
            if (itemEffect != null)
                list.add(itemEffect);
        }
        return list;
    ...
    public static void saveItemEffect(final ItemStack itemStack, final ItemEffect effect) {
        final NBTTag tag = NBTAPI.hasTag(itemStack) ? NBTAPI.getTag(itemStack) : new NBTTag();
        final NBTList mmob = tag.hasKey("mmob") ? tag.getList("mmob") : new NBTList();
    ...
    public static ItemEffect readItemEffect(final String effect) {
        ...
        final ItemEffect itemEffect = getItemEffect(components[0]);
        if (itemEffect != null) {
            // Effect must exist and must be enabled
            final Settings settings = SettingsItemEffects.getConfig(itemEffect.getName());
            if (settings == null || !settings.getBoolean("enable"))
            ...
            final String dataString = effect.replace(components[0] + ":", "");
            try {
