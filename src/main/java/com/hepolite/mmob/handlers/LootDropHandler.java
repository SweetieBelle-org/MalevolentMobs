...
public class LootDropHandler {
    ...
    public static void dropLoot(final MalevolentMob mob) {
        // Must have a valid mob
        if (mob == null)
            return;

        // Detect the loot file for the mob
        final Settings settings = SettingsRoles.getConfig(mob.getRole().lootDefinitionFile);
        if (settings == null) {
        ...
        final Set<String> itemKeys = settings.getKeys("Loot.items");
        for (final String itemKey : itemKeys) {
            final String key = settings.getString("Loot.items." + itemKey);
    ...
    private static String getMobLevelGroup(final MalevolentMob mob) {
        // Figure out the level of the mob
        final Settings settings = SettingsLoot.getConfig("Core");
        final Set<String> levelGroups = settings.getKeys("level");

        float highest = 0;
        final TreeMap<Float, String> map = new TreeMap<Float, String>();
        for (final String levelGroup : levelGroups) {
            highest = Math.max(highest, settings.getFloat("level." + levelGroup));
            map.put(settings.getFloat("level." + levelGroup), levelGroup);
        }

        final float index = mob.getLevel();
        for (final Entry<Float, String> entry : map.descendingMap().entrySet()) {
    ...
    private static String getGroupOfItem(final ItemStack item) {
        final String type = item.getType().toString().toLowerCase();
    ...
    public static ItemStack getRandomItem(final String levelGroup) {
        final Settings settings = SettingsLoot.getConfig("Loot");

        final Set<String> possibleItems = settings.getKeys(levelGroup);
        while (!possibleItems.isEmpty()) {
            ...
            final TreeMap<Float, String> map = new TreeMap<Float, String>();
            for (final String possibleItem : possibleItems) {
            ...
            final float index = totalCount * random.nextFloat();
            for (final Entry<Float, String> entry : map.entrySet()) {
                if (index < entry.getKey()) {
                    // Load up the item
                    final ItemStack item = getItemFromSettings(settings, levelGroup + "." + entry.getValue(), levelGroup);
                    if (item != null)
                        return item;
    ...
    public static ItemStack getItemFromSettings(final Settings settings, final String property, final String levelGroup) {
        try {
            final Material material = Material.getMaterial(settings.getString(property + ".material").toUpperCase());
            final int amount = settings.getInteger(property + ".amount", 1);
            final short metadata = (short) settings.getInteger(property + ".metadata", 0);
            final ItemStack item = NBTAPI.getItemStack(material, amount, metadata);
    ...
    private static void applyBaseNBTData(final ItemStack item, final Settings settings, final String property) {
        if (settings.hasProperty(property + ".nbt"))
            NBTAPI.setTag(item, settings.getTagCompound(property + ".nbt"));
    }

    private static void applyNameAndLore(final ItemStack item, final Settings settings, final String property) {
        final ItemMeta meta = item.getItemMeta();
        if (settings.hasProperty(property + ".name")) {
            final String name = settings.getString(property + ".name");
        ...
        if (settings.hasProperty(property + ".lore")) {
            final List<String> lore = settings.getStringList(property + ".lore");
    ...
    private static boolean applyItemEffects(final ItemStack item, final Settings settings, final String property, final String levelGroup) {
        final Set<String> itemEffects = settings.getKeys(property + ".itemeffects");
        for (final String itemEffect : itemEffects) {
            // Attempt to add all random item effects
            final String key = settings.getString(property + ".itemeffects." + itemEffect);
            if (key.startsWith("#random")) {
                final String group = key.contains(".") ? key.split("\\.")[1] : levelGroup;
                if (!applyRandomItemEffect(item, group))
                    return false;
            }
            // Load a specified item effect
            else {
                final String itemEffectData = SettingsLoot.readItemEffect(settings, property + ".itemeffects." + itemEffect);
                final ItemEffect effect = ItemEffectHandler.readItemEffect(itemEffectData);
    ...
    private static void applyHistory(final ItemStack item, final Settings settings, final String property) {
        final ItemMeta meta = item.getItemMeta();
        if (!meta.hasLore())
        ...
        for (final ItemEffect effect : ItemEffectHandler.getItemEffects(item)) {
            final String lore = effect.getLore();
            if (lore == null)
                continue;
    ...
    private static void applyEnchantments(final ItemStack item, final Settings settings, final String property) {
        final Set<String> enchantments = settings.getKeys(property + ".enchantments");
        for (final String enchantment : enchantments) {
            final String enchantmentName = settings.getString(property + ".enchantments." + enchantment + ".name");
            final int level = settings.getInteger(property + ".enchantments." + enchantment + ".level");
    ...
    public static boolean applyRandomItemEffect(final ItemStack item, final String levelGroup) {
        // Load up all the relevant effects in the config file
        final Settings settings = SettingsLoot.getConfig("Item_Effects");

        final Set<String> keys = settings.getKeys(levelGroup);
        final List<String> effects = new ArrayList<String>(keys.size());
        ...
        while (!effects.isEmpty()) {
            final String effect = effects.remove(random.nextInt(effects.size()));
            final ItemEffect itemEffect = ItemEffectHandler.readItemEffect(effect);
    ...
    private static class NameSection {
        ...
        public NameSection(final String section) {
        ...
        public int getNameCount(final String group) {
            int count = 0;
            for (final NameNode node : names.values())
                count += node.getNameCount(group);
            return count;
        }

        /** Returns a random name from this section */
        public String getRandomName(final String group) {
            // Sort out the nodes in numerical order, so that it is easier to pick a random name
            int totalCount = 0;

            final TreeMap<Integer, NameNode> map = new TreeMap<Integer, NameNode>();
            for (final NameNode node : names.values()) {
            ...
            // Grab one of the nodes in the tree
            final int index = random.nextInt(totalCount);
            for (final Entry<Integer, NameNode> entry : map.entrySet()) {
                if (index < entry.getKey())
    ...
    private static class NameNode {
        ...
        public int getNameCount(final String group) {
            ...
            while (trimmed.length() != 0) {
                ...
                for (final HashMap<String, NameSection> sections : links.values()) {
                    final NameSection section = sections.get(linkName.replaceAll("@group", group));
                    if (section != null) {
        ...
        public String getName(final String group) {
    ...
    private static void loadRandomNamesFromConfig(final Settings settings) {
        randomNames.clear();

        // Load up all sections in the settings file
        final Set<String> sections = settings.getKeys("");
        for (final String section : sections) {
            final Set<String> lists = settings.getKeys(section);
            if (lists.isEmpty())
        ...
        for (final NameSection section : randomNames.values()) {
            // Read up all the names in the section
            final List<String> names = settings.getStringList(section.getSectionName());
            for (final String name : names) {
                // Store the node
                final NameNode node = new NameNode(name);
                ...
                if (name.contains("#")) {
                    ...
                    while (trimmed.length() != 0) {
                        ...
                        // Store the link
                        final HashMap<String, NameSection> map = new HashMap<String, NameSection>();
                        if (linkName.contains("@group"))
                            for (final String group : nameGroups) {
                                final String modifiedLinkName = linkName.replaceAll("@group", group);
    ...
    private static String getRandomName(final ItemStack item) {
        // Grab a random item name
        final String group = getGroupOfItem(item);
        final NameSection section = randomNames.get(group);
        if (section == null)
