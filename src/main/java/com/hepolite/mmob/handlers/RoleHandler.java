...
public class RoleHandler {
    ...
    public static void loadRolesFromConfig() {
        ...
        final Set<String> roles = SettingsRoles.getRoles();
        for (final String role : roles)
            loadRoleFromConfig(SettingsRoles.getConfig(role), role);
    }

    /** Loads up the specified role */
    private static void loadRoleFromConfig(final Settings settings, final String name) {
        // Need a valid role
        ...
        // Create the new role
        final MobRole mobRole = new MobRole(name);
