...
public class InstructionDungeonSetBbox extends Instruction {
    ...
    protected boolean onInvoke(final CommandSender sender, final List<String> arguments) {
        ...
        final List<Integer> numbers = parseInts(arguments);
        if (numbers.size() == 6) {
            final Vector min = new Vector(Math.min(numbers.get(0), numbers.get(3)), Math.min(numbers.get(1), numbers.get(4)), Math.min(numbers.get(2), numbers.get(5)));
            final Vector max = new Vector(Math.max(numbers.get(0), numbers.get(3)), Math.max(numbers.get(1), numbers.get(4)), Math.max(numbers.get(2), numbers.get(5)));
            if (spawner == null) {
            ...
            final Vector v = new Vector(numbers.get(0), numbers.get(1), numbers.get(2));
            if (spawner == null) {
                if (dungeon.getBoundingBoxMin() == null)
                    dungeon.setBoundingBox(v, null);
                else if (dungeon.getBoundingBoxMax() == null) {
                    final Vector b = dungeon.getBoundingBoxMin();
                    final Vector min = new Vector(Math.min(v.getX(), b.getX()), Math.min(v.getY(), b.getY()), Math.min(v.getZ(), b.getZ()));
                    final Vector max = new Vector(Math.max(v.getX(), b.getX()), Math.max(v.getY(), b.getY()), Math.max(v.getZ(), b.getZ()));
                    dungeon.setBoundingBox(min, max);
                } else
                    dungeon.setBoundingBox(v, null);
                ...
                if (spawner.getBoundingBoxMin() == null)
                    ...
                    final Vector b = spawner.getBoundingBoxMin();
                    final Vector min = new Vector(Math.min(v.getX(), b.getX()), Math.min(v.getY(), b.getY()), Math.min(v.getZ(), b.getZ()));
                    final Vector max = new Vector(Math.max(v.getX(), b.getX()), Math.max(v.getY(), b.getY()), Math.max(v.getZ(), b.getZ()));
                    spawner.setBoundingBox(min, max);
                } else
