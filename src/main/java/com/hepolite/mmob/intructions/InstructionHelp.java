...
public class InstructionHelp extends Instruction {
    ...
    protected boolean onInvoke(final CommandSender sender, final List<String> arguments) {
        if (arguments.size() == 0)
            ...
            final String argument = compressList(arguments, " ");
    ...
    private void displayInstructionInformation(final CommandSender sender, int page) {
        // Grab all instructions
        final List<Instruction> instructions = CommandHandler.getInstructions();
        page = Math.max(0, page);

        sender.sendMessage(ChatColor.WHITE + "Displaying page " + (page + 1) + " of " + (instructions.size() / 10 + 1) + " pages of instructions.");
        ...
        for (int i = 10 * page; i < Math.min(10 * (page + 1), instructions.size()); i++)
    ...
    private boolean displayInstructionHelp(final CommandSender sender, final String instructionName) {
        // Grab all instructions
        final List<Instruction> instructions = CommandHandler.getInstructions();
        for (final Instruction instruction : instructions)
            if (instruction.getFullName().equalsIgnoreCase(instructionName)) {
                // Grab all the data for the instruction
                final List<String> syntaxes = new ArrayList<String>();
