...
public class ItemEffectCharged extends ItemEffect {
    ...
    public void onAttacking(final EntityDamageByEntityEvent event, final Player player, final ItemStack item) {
        ...
        // Calculate damages
        final double damage = (1.0 + damagePercentBoost) / targets * event.getDamage();
        double totalDamage = damage;
        event.setDamage(damage);

        // Grab all entities within the area and randomly select up to "targets" amount of them
        final List<LivingEntity> entities = Common.getEntitiesInRange(event.getEntity().getLocation(), range);
