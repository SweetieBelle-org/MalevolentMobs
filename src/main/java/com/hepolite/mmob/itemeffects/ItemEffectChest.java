...
public abstract class ItemEffectChest extends ItemEffect {
    ...
    protected ItemEffectChest(final String name) {
        super(name);
    }
    ...
    public void onRightClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        final BlockPlaceEvent blockPlaceEvent = new BlockPlaceEvent(state.getBlock(), state, block, new ItemStack(Material.CHEST), player, true, EquipmentSlot.HAND);
    ...
    private void performEasterEgg(final Inventory inventory, final Location location) {
        if (random.nextFloat() > 0.075f)
            return;

        // Find a random player in the vicinity and add a skull of that player
        final List<Player> players = Common.getPlayersInRange(location, 20.0f);
        if (players.size() == 0)
            return;
        ...
        final ItemStack skull = NBTAPI.setTag(new ItemStack(Material.SKULL_ITEM, 1, (short) 3), tag);
        ...
        final String[] lore = getEasterEggLore();
        final List<String> list = new LinkedList<String>();
        final String reset = "&r&8";
        final String[] noise = new String[] { "&k", "&k", "&k", "&0", "&0", "&l", "&4", "&o" };
