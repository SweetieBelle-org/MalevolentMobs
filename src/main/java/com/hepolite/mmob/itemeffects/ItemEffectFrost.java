...
public class ItemEffectFrost extends ItemEffect {
    ...
    public void addDescription(final List<String> list) {
        list.add(String.format("&fSlows down targets by &b%d%%&f for &b%.1f&f seconds", 15 * strength, duration / 20.0f));
        if (damage != 0.0f)
    ...
    public void loadFromString(final String dataString) {
        ...
        strength = Math.round(Float.parseFloat(components[0]) / 15.0f);
