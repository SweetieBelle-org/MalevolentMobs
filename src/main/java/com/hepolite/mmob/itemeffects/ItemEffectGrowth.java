...
public class ItemEffectGrowth extends ItemEffect {
    ...
    public void onRightClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        final Block centerBlock = event.getClickedBlock();
        final Material centerType = centerBlock.getType();
        final Byte meta = centerBlock.getData();

        // Convert dirt to grass
        if (centerType == Material.DIRT) {
            final BlockBreakEvent logBlockEvent = new BlockBreakEvent(centerBlock, player);
        ...
        // Grow flowers/tallgrass on grass blocks
            ...
            final PlayerInteractEvent bonemealEvent = new PlayerInteractEvent(player, Action.RIGHT_CLICK_BLOCK, new ItemStack(Material.INK_SACK, 1, (short) 15), centerBlock, event.getBlockFace());
            ...
            if (!bonemealEvent.isCancelled()) {
                ...
                for (int x = -2; x <= 2; x++)
                    for (int z = -2; z <= 2; z++)
                        for (int y = 0; y <= 2; y++) {
                            ...
                            final Block block = centerBlock.getRelative(x, y, z);
                ...
                if (flowersGrown > 0)
                    damageItem(item, durabilityCostPerFlower * flowersGrown);
            }
        }
        ...
        // Progress crops' growth
            ...
            {
                final Byte newMeta = (byte) Math.min(7, meta + random.nextInt(3) + 1);
                damageItem(item, (newMeta - meta) * durabilityCostPerCrop);
