...
public class ItemEffectHarvest extends ItemEffect {
    ...
    public void onLeftClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        if (centerType == Material.DIRT || centerType == Material.GRASS || centerType == Material.MYCEL) {
                ...
                for (int z = -2; z <= 2; z++)
                    for (int y = -1; y <= 1; y++) {
                        ...
                        if ((type == Material.DIRT || type == Material.GRASS || type == Material.MYCEL) && block.getRelative(0, 1, 0).getType() == Material.AIR) {
                            final BlockBreakEvent logBlockEvent = new BlockBreakEvent(block, player);
            ...
            if (blocksTilled > 0)
                damageItem(item, durabilityCostPerTill * blocksTilled);
        }
        ...
        // Find crops to harvest and replant
            ...
            final Location location = centerBlock.getLocation().add(0.5, 1.0, 0.5);
            if (wheat != 0)
            ...
            if (blocksHarvested > 0)
                damageItem(item, durabilityCostPerHarvest * blocksHarvested);
        }
