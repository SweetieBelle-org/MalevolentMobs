...
public class ItemEffectLacerate extends ItemEffect {
    ...
    public void onAttacking(final EntityDamageByEntityEvent event, final Player player, final ItemStack item) {
        // Need a valid entity
        final LivingEntity entity = (LivingEntity) (event.getEntity() instanceof LivingEntity ? event.getEntity() : null);
        if (entity == null)
            return;

        @SuppressWarnings("deprecation")
        final double additionalDamage = percentMax * entity.getMaxHealth() + percentLeft * entity.getHealth();
