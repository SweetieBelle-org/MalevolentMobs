...
public class ItemEffectPowerline extends ItemEffectWand {
    ...
    public void onLeftClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        final Location end = start.clone().add(start.getDirection().multiply(lightningBoltDistanceCheck));
        final Location location = Common.getObstruction(start, end, player);
        if (location == null)
            return;

        for (final LivingEntity entity : Common.getEntitiesInLocation(location))
    ...
    public void onRightClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        final Location start = player.getEyeLocation();
        final Location end = start.clone().add(start.getDirection().multiply(lightningStrikeDistanceCheck));
        final Location location = Common.getObstruction(start, end, player);
        if (location == null)
            return;
        ...
        Common.createLightningStrike(location, chargeCostPerLightningStrike, lightningStrikeRadius, false);
    }
