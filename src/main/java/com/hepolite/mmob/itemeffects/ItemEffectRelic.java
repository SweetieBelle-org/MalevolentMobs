...
public class ItemEffectRelic extends ItemEffectChest {
    ...
    protected void addItems(final Inventory inventory, final Location location) {
        final float chance = value - (float) Math.floor(value);
        final int count = (int) Math.floor(value) + (random.nextFloat() < chance ? 1 : 0);
        for (int i = 0; i < count; i++) {
            final ItemStack item = LootDropHandler.getRandomItem(group);
