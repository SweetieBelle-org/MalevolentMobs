...
public class ItemEffectSharpness extends ItemEffect {
    ...
    public void onAttacking(final EntityDamageByEntityEvent event, final Player player, final ItemStack item) {
        final boolean isMalevolentTarget = event.getEntity() instanceof LivingEntity && MobHandler.isMobMalevolent((LivingEntity) event.getEntity());

        final double factor = (isMalevolentTarget ? 1.0 + percentMob : 1.0) * (1.0 + percentAll);
        final double newDamage = factor * event.getDamage() + (isMalevolentTarget ? flatMob : 0.0f) + flatAll;
