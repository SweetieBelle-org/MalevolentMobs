...
public class ItemEffectTimber extends ItemEffect {
    ...
    public void onRightClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        final float factor = durabilityCostPerBlock < 1.0f ? 1.0f : durabilityCostPerBlock;
        int chopsLeft = Math.min(item.getType().getMaxDurability() - (int) (item.getDurability() / factor), strength);
        int blocksChopped = 0;
        final Material material = block.getType();
        final Byte data = (byte) (block.getData() % 4);

        // Will chop down blocks that are closest to the player first
        final List<Location> blocksInSortedOrder = new LinkedList<Location>();
        ...
        while (chopsLeft > 0 && blocksInSortedOrder.size() != 0) {
            ...
            final Location location = blocksInSortedOrder.remove(0);
            ...
            for (int x = -1; x <= 1; x++)
                for (int z = -1; z <= 1; z++)
                    for (int y = -1; y <= 1; y++) {
                        if (x == 0 && y == 0 && z == 0)
                            continue;
                        final Block newBlock = location.getWorld().getBlockAt(location.getBlockX() + x, location.getBlockY() + y, location.getBlockZ() + z);
            ...
            // Chop down the block
            chopsLeft--;
            final BlockBreakEvent logBlockEvent = new BlockBreakEvent(location.getBlock(), player);
        ...
        if (blocksChopped > 0) {
            ...
            damageItem(item, durabilityCostPerBlock * blocksChopped);
        }
    }

    @Override
    public boolean canBeUsedOnItem(final ItemStack item) {
        final String materialType = item.getType().toString().toLowerCase().replace("pickaxe", "");
