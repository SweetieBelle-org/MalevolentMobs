...
public class ItemEffectTunneler extends ItemEffect {
    ...
    public void onRightClick(final PlayerInteractEvent event, final Player player, final ItemStack item) {
        ...
        for (int x = -1; x <= 1; x++)
            for (int z = -1; z <= 1; z++)
                for (int y = -1; y <= 1; y++) {
                    ...
                    final Block newBlock = block.getWorld().getBlockAt(block.getX() + x, block.getY() + y, block.getZ() + z);
                    if (newBlock.getType() != Material.STONE && newBlock.getType() != Material.COBBLESTONE)
                        continue;

                    final BlockBreakEvent logBlockEvent = new BlockBreakEvent(newBlock, player);
                    Common.postEvent(logBlockEvent);
                    if (!logBlockEvent.isCancelled()) {
                        blocksMined++;
                        for (final ItemStack itemToDrop : newBlock.getDrops(item))
        ...
        if (blocksMined > 0)
            damageItem(item, blocksMined * durabilityCostPerBlock);
    }
