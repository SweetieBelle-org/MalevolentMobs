...
public abstract class ItemEffectWand extends ItemEffectCharger {
    ...
    protected ItemEffectWand(final String name) {
        super(name);
    ...
    protected final boolean validateCooldown(final Player player) {
        final int time = getCooldownTime(player.getUniqueId());
        if (time > 0) {
            player.sendMessage(ChatColor.translateAlternateColorCodes('&', String.format("&fWand is charging, need &c%.1f&f more seconds", time / 20.0f)));
            return false;
        }
        return true;
    }
    ...
    protected final void setCooldownTime(final UUID user, final int time) {
        // Grab a cooldown node
        if (!cooldownNodes.containsKey(user))
            cooldownNodes.put(user, new CooldownNode());
        final CooldownNode node = cooldownNodes.get(user);
    ...
    protected final int getCooldownTime(final UUID user) {
        final CooldownNode node = cooldownNodes.get(user);
    ...
    protected final boolean isCooldownTimeOver(final UUID user) {
