...
public class SettingsLoot
{
	...
	public static void initialize()
	{
		...
		File directory = new File(MMobPlugin.getInstance().getDataFolder() + "/Loot");
		if (!directory.exists())
		{
			directory.mkdir();
			addDefaultSettings();
	...
	public static Settings getConfig(String category)
	{
		Settings setting = categories.get(category);
		if (setting == null)
		{
	...
	{
		Settings setting = new Settings("Loot", "Core.yml");
	...
	{
		Settings setting = new Settings("Loot", "Names.yml");
	...
	{
		Settings setting = new Settings("Loot", "Item_Effects.yml");
		...
		writeItemEffect(setting, name + i++, "Fiery", "i40/50");
		...
		writeItemEffect(setting, name + i++, "Fiery", "i50/60");
		...
		writeItemEffect(setting, name + i++, "Fiery", "i60/80");
		...
		writeItemEffect(setting, name + i++, "Fiery", "i100/125");
		...
		writeItemEffect(setting, name + i++, "Fiery", "i180/200");
	...
	{
		Settings setting = new Settings("Loot", "Loot.yml");

		String[] groups;
		String name;
		int i;
		List<String[]> effectList = new LinkedList<String[]>();
		List<Object[]> enchantmentList = new LinkedList<Object[]>();
	...
	public static void writeItem(Settings setting, String property, Material material, int metadata, int amount, float weight, int randomItemEffects, int randomNegativeItemEffects)
	{
	...
	/** Writes a complex item to the given settings file, under the given property */
	public static void writeItem(Settings setting, String property, Material material, int metadata, int amount, float weight, String name, String[] lore, String history, int randomItemEffects, int randomNegativeItemEffects, List<String[]> itemEffectData, List<Object[]> enchantmentData, NBTTag tag)
	{
		...
		{
			for (String[] data : itemEffectData)
			{
		...
		if (enchantmentData != null)
		{
			...
			for (Object[] data : enchantmentData)
	...
	public static void writeItemEffect(Settings setting, String property, String name, String... parameters)
	{
		setting.addDefault(property + ".name", name);
	...
	public static String readItemEffect(Settings setting, String property)
	{
		String effect = setting.getString(property + ".name");
		Set<String> parameters = setting.getKeys(property + ".parameters");
		Object[] parsedParameters = new Object[parameters.size()];
