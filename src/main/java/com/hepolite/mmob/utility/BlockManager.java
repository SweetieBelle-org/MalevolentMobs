...
public class BlockManager {
    ...
    public static void onTick() {
        ...
        for (final Iterator<BlockNode> it = nodes.iterator(); it.hasNext();) {
            final BlockNode node = it.next();
            if (--node.duration < 0) {
                node.remove();
                it.remove();
            }
    ...
    public static void setBlock(final World world, final int x, final int y, final int z, final Material material, final boolean replaceEverything, final int duration) {
    ...
    public static void setBlock(final World world, final int x, final int y, final int z, final Material material, final byte metaData, final boolean replaceEverything, final int duration) {
        // Generate a new node
        final BlockNode node = new BlockNode(material, metaData, world, x, y, z, replaceEverything, duration);
        if (node.didSomething && duration != -1)
            nodes.add(node);
    }

    /** Returns true if the location is a changed block */
    public static boolean isLocationNoted(final Location location) {
    ...
    private static class BlockNode {
        ...
        public BlockNode(final Material newMaterial, final Byte metaData, final World world, final int x, final int y, final int z, final boolean replaceBlocks, final int duration) {
            // Set the variables
